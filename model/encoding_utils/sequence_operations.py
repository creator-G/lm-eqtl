...
def random_masking(
    ...
    if split_mask:
        tmp_masker = [one_hot(mapping[np.random.choice(list(bases))]) for i in range(idx_random-idx_masked)]
...
def random_masking_weighted(
    ...
    (80% with mask token, 10% random, 10% keep)
    if frame given, we do not mask the edges
    Perform masking individually for each class
    """
    ...
    framed_seq = seq_vector[frame:len(seq_vector)-frame]
    framed_targets = seq_labels[frame:len(seq_vector)-frame]
    
    for class_idx in range(Nclasses):
        class_mask = framed_targets == class_idx
        framed_seq[class_mask], framed_mask[class_mask] = random_masking(framed_seq[class_mask], split_mask=split_mask, masker=masker, mask_rate=mask_rate[class_idx], frame=0)
    
    seq_vector[frame:len(seq_vector)-frame] = framed_seq
    mask[frame:len(seq_vector)-frame] = framed_mask
...
def encode_and_pad(
    ...
    if isinstance(mask_rate,Iterable) and len(mask_rate)==1:
        mask_rate = mask_rate[0]

    # mask the sequence, masked sequence remains one hot encoded
    if mask is not None:
        masked_seq = seq_one_hot
        if split_mask == True:
            idx_masked = np.where(mask)[0]
            np.random.shuffle(idx_masked)
            N_idx = len(idx_masked)
            masked_seq[idx_masked[0:int(0.8*N_idx)]] = np.zeros(Nclasses)
        ...
        mask_rate.append(mask_rate_N)
        masked_seq, mask = random_masking_weighted(seq_one_hot,seq_labels,
                                Nclasses = Nclasses,
                                mask_rate=mask_rate,
    ...
    seq_labels = mask_buffer(seq_labels, buffer[0], buffer[1], ignore_index=-100.0)
...
def rolling_mask(seq_one_hot, stride, masker=np.zeros(Nclasses), frame=0):
    """
...
def rolling_mask_test(seq, motifs={}, mask_stride=30, masker=np.zeros(Nclasses), frame=0):
    """
    Create all needed sequences for testing
    """
    # truncate all to same length
...
def rolling_exo_mask_test(seq_data, mask_stride=30, masker=np.zeros(Nclasses), frame=0):
    """
...
def motif_mask_encoding(seq,motifs,seq_len=300,total_len=303,masker=np.zeros(Nclasses)):
    """
...
def one_hot_and_pad(seq, length, max_len):
    if length == max_len:
        ...
        return np.concatenate([one_hot_(seq[:length]),np.zeros((max_len-length,Nclasses))], axis=0)
